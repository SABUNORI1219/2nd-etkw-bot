...
def draw_decorative_frame(img: Image.Image,
    ...
    """
    外枠（太）＋内枠（細）を描画します。
    アーチは arcs の stroke 実装に依存せず、必ず見える形で描画するために
    pieslice-fill + 内部切り抜きで「stroke」を作る方式を採用しています。
    直線の位置は一切変えず、アーチの bbox を直線アンカーに揃えることで
    他処理に影響を与えません。
    """
    w, h = img.size
    ...
    # helper to ensure bbox is inside image and non-degenerate
    ...
    # helper: draw a stroked arc reliably by filling a pieslice and clearing inner area
    def _draw_stroked_arc(layer_draw: ImageDraw.ImageDraw, bbox, start, end, color, stroke_w):
        cb = _clamp_bbox(bbox)
        try:
            layer_draw.pieslice(cb, start=start, end=end, fill=color)
        except Exception:
            # fallback to arc stroke (best-effort)
            try:
                layer_draw.arc(cb, start=start, end=end, fill=color, width=stroke_w)
                return
            except Exception:
                return
        # carve out inner area to create ring (stroke) effect
        inner = [cb[0] + stroke_w, cb[1] + stroke_w, cb[2] - stroke_w, cb[3] - stroke_w]
        if inner[2] > inner[0] and inner[3] > inner[1]:
            try:
                layer_draw.pieslice(inner, start=start, end=end, fill=(0, 0, 0, 0))
            except Exception:
                try:
                    layer_draw.ellipse(inner, fill=(0, 0, 0, 0))
                except Exception:
                    pass

    # compute straight-line anchors first (these define where lines sit; DO NOT change them)
    ...
    # Left-top arc: top point at top_y, center x at left_x
    left_arc_box = [
    ...
    # Left-bottom arc: bottom aligned to bot_y
    bottom_left_arc_box = [
        int(math.floor(left_x - r)),
    ...
    # compute arc edge points (used for trimming straight lines)
    ...
    # --- draw on separate frame_layer ---
    frame_layer = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    draw_frame = ImageDraw.Draw(frame_layer)

    # draw outer straight trimmed lines onto frame_layer (positions unchanged)
    top_y = oy + int(outer_width / 2) + line_inset_outer
    bot_y = oy + oh - int(outer_width / 2) - line_inset_outer
    ...
    # --- erase regions under outer arcs from frame_layer via mask so straight lines don't show through arcs ---
    ...
    # --- draw outer arcs robustly using pieslice-based stroked arc ---
    _draw_stroked_arc(draw_frame, left_arc_box, 0, 90, frame_color, outer_width)
    _draw_stroked_arc(draw_frame, right_arc_box, 90, 180, frame_color, outer_width)
    _draw_stroked_arc(draw_frame, bottom_right_arc_box, 180, 270, frame_color, outer_width)
    _draw_stroked_arc(draw_frame, bottom_left_arc_box, 270, 360, frame_color, outer_width)

    # --- inner straight lines drawn onto frame_layer (positions unchanged) ---
    in_overlap = 0
    inner_top_y = iy + int(inner_width / 2) + line_inset_inner
    inner_bot_y = iy + ih - int(inner_width / 2) - line_inset_inner

    # horizontal inner lines fixed to inner_top_y/inner_bot_y
    ...
    # draw inner arcs robustly using pieslice-based stroked arc
    _draw_stroked_arc(draw_frame, li_box, 0, 90, (95, 60, 35, 220), inner_width)
    _draw_stroked_arc(draw_frame, ri_box, 90, 180, (95, 60, 35, 220), inner_width)
    _draw_stroked_arc(draw_frame, br_box, 180, 270, (95, 60, 35, 220), inner_width)
    _draw_stroked_arc(draw_frame, bl_box, 270, 360, (95, 60, 35, 220), inner_width)

    # composite frame_layer onto base image and return
    out = img.convert("RGBA")
