...
def generate_profile_card(info, output_path="profile_card.png", other_api=None):
    try:
    ...
    # --- 非同期API呼び出しを分離 ---
    skin = None
    if uuid and other_api is not None:
        try:
            skin = info.get("_skin_image")  # 事前に取得されている場合
            if skin is None:
                skin = other_api.get_vzge_skin_image_sync(uuid, size=196)
        except Exception as e:
    ...
    draw.text((585, 1400), "Generated by", font=font_mini, fill=(60,40,30,255))
    draw.text((635, 1435), "Minister Chikuwa#5740", font=font_mini, fill=(60,40,30,255))
    try:
...
# 非同期API呼び出し用
async def fetch_profile_skin_image(uuid, other_api, size=196):
    skin = None
    try:
        skin = await other_api.get_vzge_skin_image(uuid, size=size)
    except Exception as e:
        logger.error(f"Skin image load failed: {e}")
        skin = None
    return skin
