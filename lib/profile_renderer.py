...
def generate_profile_card(info, output_path="profile_card.png"):
    ...
    draw.text((330, 410), "", font=font_main, fill=(60,40,30,255))
    draw.text((330, 485), "", font=font_main, fill=(60,40,30,255))

    draw.text((90, 600), f"First Join: {info.get('first_join', 'N/A')}", font=font_sub, fill=(60,40,30,255))
    draw.text((90, 675), f"Last Seen: {info.get('last_join', 'N/A')}", font=font_sub, fill=(60,40,30,255))
    ...
    draw.text((650, 1150), wars_text, font=font_raids, fill=(60,40,30,255))
    bbox = draw.textbbox((650, 1150), wars_text, font=font_sub)
    x_wars = bbox[2] + 6
    draw.text((x_wars, 1150 + 18), f" #{info.get('war_rank_display', 'N/A')}", font=font_mini, fill=(60,40,30,255))

    draw.text((475, 1200), "World Events", font=font_raids, fill=(60,40,30,255))
    draw.text((650, 1200), f"{fmt_num(info.get('world_events', 0))}", font=font_raids, fill=(60,40,30,255))
    ...
    draw.text((525, 1400), "Generated by Minister Chikuwa#5740", font=font_uuid, fill=(90,90,90,255))
